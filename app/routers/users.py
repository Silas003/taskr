...
from app.schemas.UserSchema import SystemRole, ProjectRole
...
@router.get("",dependencies=[Depends(get_current_user),Depends(require_system_role(SystemRole.admin))])
def get_all_users(skip:int = Query(default=0,ge=0),limit:int = Query(default=10,ge=1,le=100),service:UserService=Depends(get_user_service)):
    users  = service.get_all_users(skip,limit)
    return  ResponseBase(code=HTTPStatus.OK,message="Users retrieved successfully",data=UserRead(**users.__dict__))
...
def register_user(
    ...
    try:
        ...
        return ResponseBase[UserRead](code=HTTPStatus.CREATED, message="User registered successfully", data=UserRead(**user.__dict__))
...
@router.get("/me", response_model=ResponseBase[UserRead],dependencies=[Depends(user_access_control)])
def read_own_user(current_user = Depends(get_current_user)) -> ResponseBase[UserRead]:
    """Get the current authenticated user's profile."""
    return ResponseBase(code=HTTPStatus.OK, message="User profile retrieved successfully", data=UserRead(**current_user.__dict__))
...
@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT,dependencies=[Depends(get_current_user),Depends(user_access_control)])
